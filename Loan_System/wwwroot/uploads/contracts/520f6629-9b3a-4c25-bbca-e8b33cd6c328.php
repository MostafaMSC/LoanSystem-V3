...
if ($ChosenDep) {
    $sql = "SELECT * FROM daewaaa WHERE office_name = ? ORDER BY id DESC LIMIT ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("si", $ChosenDep, $RecordNumber);
} else {
    $sql = "SELECT * FROM daewaaa ORDER BY id DESC LIMIT ?";
    $stmt = $conn->prepare($sql);
...
$totalCasesQuery = $conn->query("SELECT COUNT(*) as total FROM daewaaa");
$totalCases = $totalCasesQuery->fetch_assoc()['total'];

$casesByOffice = [];
$res = $conn->query("SELECT office_name, COUNT(*) as total FROM daewaaa GROUP BY office_name");
...
$casesByYear = [];
$res = $conn->query("SELECT DawaYear, COUNT(*) as total FROM daewaaa GROUP BY DawaYear ORDER BY DawaYear DESC");
while ($row = $res->fetch_assoc()) {
    $casesByYear[$row['DawaYear']] = $row['total'];
}

$haladawaaQuery = $conn->query("SELECT haladawaa, COUNT(*) as total FROM daewaaa GROUP BY haladawaa");
$casesByStatus = [];
    ...
    <div class="container">
        ...
        <div class="stats-container">
            <div class="stat-card">
                ...
                <div class="stat-label">   إجمالي الدعاوى المدنية</div>
            </div>
...
function printTable() {
    ...
    newWin.print();
        ...
        function exportToPDF() {
            var document = document.getElementById('table-container');
            document.print();
